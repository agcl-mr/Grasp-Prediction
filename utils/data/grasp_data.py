...
class GraspDatasetBase(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.random_rotate:
            rotations = [0, np.pi / 2, 2 * np.pi / 2, 3 * np.pi / 2]   #pick randomly out of 4 rotation choice
            rot = random.choice(rotations)
        else:
            rot = 0.0

        if self.random_zoom:
            zoom_factor = np.random.uniform(0.5, 1.0)      #pick randomly from uniform distribution
        else:
            zoom_factor = 1.0
